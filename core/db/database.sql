...
-- =============================================================================
CREATE TABLE `migration_tn` (`schema_version` smallint(4) NOT NULL)
...
-- =============================================================================
ALTER TABLE `category_tn` ADD CONSTRAINT `category_tn__parent` FOREIGN KEY (`parent`)
	REFERENCES `category` (`id`) ON DELETE SET NULL ON UPDATE CASCADE;
ALTER TABLE `page_tn` ADD CONSTRAINT `page_user` FOREIGN KEY (`user`)
	REFERENCES `Wo_Users` (`user_id`) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE `post_tn` ADD CONSTRAINT `post_user` FOREIGN KEY (`user`)
	REFERENCES `Wo_Users` (`user_id`) ON DELETE CASCADE ON UPDATE CASCADE;
ALTER TABLE `post_tn` ADD CONSTRAINT `post_category` FOREIGN KEY (`category`)
	REFERENCES `category` (`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- =============================================================================
-- Insert some demo data
-- =============================================================================
/*INSERT INTO `Wo_Users` (`id`, `time_insert`, `time_update`, `slug`, `username`, `password`,`email`, `fullname`, `mailaddr`, `body`, `argv`) VALUES
(4, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 'john-doe', 'ChangeMe', '$2y$10$jH48L1K1y9dB303aI2biN.ob0biZDuUbMxPKadi3wDqOIxj6yNT6K', 'John Doe', 'mail@example.org', 'Hello, I\'m John Doe! 😎\r\n\r\nA user like me can currently considered more of an *author* which can be assigned to many posts (and pages). There is no user permission system in the administration area currently, so every user with valid login credentials can do anything!', NULL);
*/

INSERT INTO `category_tn` (`id`, `time_insert`, `time_update`, `parent`, `slug`, `name`, `body`, `argv`) VALUES
...
INSERT INTO `page_tn` (`id`, `time_insert`, `time_update`, `user`, `slug`, `name`, `body`, `argv`) VALUES
...
INSERT INTO `post_tn` (`id`, `time_insert`, `time_update`, `user`, `category`, `slug`, `name`, `body`, `argv`) VALUES
